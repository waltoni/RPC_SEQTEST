...
public class FlippedCustomScript : BaseNetLogic
{
    private const int MinCount = 1;
    private const int MaxCount = 62;

    private const string BaseBoxTemplateName = "FBaseBox1";
    private const string CaseLocationsTemplateName = "FCaseLocations1";

    private const string CustomPatternPath = "Model/RecipeEdit/CustomPatternFlipped";
    ...
    private void Rebuild()
    {
        ...
        // Initialize WidthArray and HeightArray when display loads (flipped copies NumCasesCustom from normal)
        ...
        int currentCount = CountExistingElements(baseBoxContainer, "FBaseBox");

        for (int n = currentCount + 1; n <= targetCount; n++)
        {
            try
            {
                if (baseBoxContainer.Get($"FBaseBox{n}") == null)
                {
                    var newBaseBox = InformationModel.MakeObject($"FBaseBox{n}", baseBox1.ObjectType.NodeId);
                ...
                if (caseLocationsContainer.Get($"FCaseLocations{n}") == null)
                {
                    var newCaseLocations = InformationModel.MakeObject($"FCaseLocations{n}", caseLocations1.ObjectType.NodeId);
            ...
            {
                Log.Warning(nameof(FlippedCustomScript), $"Rebuild: failed for index {n}: {ex.Message}");
            }
        }
    ...
    public void Add()
    {
        ...
        if (baseBox1 == null || caseLocations1 == null)
        {
            Log.Error(nameof(FlippedCustomScript), $"Templates '{BaseBoxTemplateName}' or '{CaseLocationsTemplateName}' not found.");
            return;
        }
        ...
        if (baseBoxContainer == null || caseLocationsContainer == null)
        {
            Log.Error(nameof(FlippedCustomScript), "Could not resolve FBaseBox or FCaseLocations container.");
            return;
        }
        ...
        {
            Log.Error(nameof(FlippedCustomScript), $"Add failed: {ex.Message}");
        }
    }
    ...
    public void Subtract()
    {
        ...
        var baseBoxToRemove = baseBoxContainer.Get($"FBaseBox{count}");
        var caseLocationsToRemove = caseLocationsContainer.Get($"FCaseLocations{count}");
    ...
    public void Snap()
    {
        Log.Info(nameof(FlippedCustomScript), "Snap() called");
        ...
        if (!GetSnapMode())
        {
            Log.Info(nameof(FlippedCustomScript), "Snap: SnapMode off, returning");
            return;
        }
        ...
        if (cp == null)
        {
            Log.Warning(nameof(FlippedCustomScript), "Snap: CustomPatternFlipped not found");
            return;
        }
        ...
        {
            Log.Info(nameof(FlippedCustomScript), $"Snap: early return (CaseSelected={i}, NumCases={numCases})");
            return;
        }
        ...
        Log.Info(nameof(FlippedCustomScript), $"Snap: applied for case {i} -> ({bestLeft:F1}, {bestTop:F1})");
    ...
    private void ApplyBoundaryCheck()
    {
        ...
        if (Math.Abs(clampedLeft - currLeft) > 0.001f || Math.Abs(clampedTop - currTop) > 0.001f)
        {
            ...
            Log.Info(nameof(FlippedCustomScript), $"BoundaryCheck: case {i} clamped from ({currLeft:F0},{currTop:F0}) to ({clampedLeft:F0},{clampedTop:F0}) | limits px: left[{leftMin:F0},{leftMax:F0}] top[{topMin:F0},{topMax:F0}] extW={extW:F0} extH={extH:F0}");
        }
    }
    ...
    private void InitializeWidthHeightArrays(int targetCount)
    {
        ...
        {
            Log.Warning(nameof(FlippedCustomScript), $"InitializeWidthHeightArrays failed: {ex.Message}");
        }
    }
    ...
    private void InitializeArraysForIndex(int n)
    {
        ...
        {
            Log.Warning(nameof(FlippedCustomScript), $"Array init for index {n} failed: {ex.Message}");
        }
    }
    ...
    private void SetArrayElement(IUAVariable arrayVar, int index, float value)
    {
        ...
        {
            Log.Warning(nameof(FlippedCustomScript), $"SetArrayElement failed: {ex.Message}");
        }
    }
    ...
    private void CopyArrayElement(IUAVariable arrayVar, int fromIndex, int toIndex)
    {
        ...
        {
            Log.Warning(nameof(FlippedCustomScript), $"CopyArrayElement failed: {ex.Message}");
        }
    }
