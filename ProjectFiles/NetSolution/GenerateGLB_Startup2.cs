...
public class GenerateGLB_Startup2 : BaseNetLogic
{
    ...
    public void CreateGLB()
    {
        ...
        var currentNameVar = LogicObject.GetVariable("CurrentFileName2");
        if (currentNameVar == null)
        {
            currentNameVar = InformationModel.MakeVariable("CurrentFileName2", OpcUa.DataTypes.String);
            LogicObject.Add(currentNameVar);
            currentNameVar.Value = "pallet_layout_A2";
        }

        string current = currentNameVar.Value;
        string next = current == "pallet_layout_A2" ? "pallet_layout_B2" : "pallet_layout_A2";
        ...
        // ------------------------------------------------------------------
        // 🔹 Update HTML to proper GLB file (A2 or B2)
        // ------------------------------------------------------------------
        var browser = Owner.Get<WebBrowser>("WebBrowser");
        browser.Visible = false;

        string templatePath = new ResourceUri(LogicObject.GetVariable("TempFile2")?.Value ?? LogicObject.GetVariable("TempFile").Value).Uri;
        string filePath = new ResourceUri(LogicObject.GetVariable("DestFile2")?.Value ?? LogicObject.GetVariable("DestFile").Value).Uri;

        string text = File.ReadAllText(templatePath);

        string currentNameHtml = LogicObject.GetVariable("CurrentFileName2")?.Value ?? "pallet_layout_A2";
        Log.Info($"🔄 Loading GLB: {currentNameHtml}.glb");

        text = text.Replace("$File", currentNameHtml);

        File.WriteAllText(filePath, text);
        Log.Info($"✅ index2.html regenerated at: {filePath}");
