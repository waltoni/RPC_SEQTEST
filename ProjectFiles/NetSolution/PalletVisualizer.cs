...
public class PalletVisualizer : BaseNetLogic
{
    ...
    private IUAVariable unitXVar;
    private IUAVariable unitYVar;
    private IUAVariable unitRVar;
    ...
    private IUAVariable seqUnitNumVar;

    public override void Start()
    {
        // Cache object and variable references once at startup
        svgImage = LogicObject.Owner.Get<AdvancedSVGImage>("AdvancedSVGImageNormal");
        paramObj = LogicObject.Owner.GetObject("PalletVisualizationParameters");
        
        if (paramObj != null)
        {
            palletWidthVar = paramObj.GetVariable("PalletWidth");
            palletLengthVar = paramObj.GetVariable("PalletLength");
            unitXVar = paramObj.GetVariable("UnitX");
            unitYVar = paramObj.GetVariable("UnitY");
            unitRVar = paramObj.GetVariable("UnitR");
            ...
            seqUnitNumVar = paramObj.GetVariable("Seq_UnitNum");
        }
    }
    ...
    public void replaceSVG()
    {
        ...
        {
            Log.Error("PalletVisualizer", "Cached references not initialized. Call Start() first.");
            return;
        }
        ...
        float[] unitX = unitXVar.Value;
        float[] unitY = unitYVar.Value;
        float[] unitR = unitRVar.Value;
        ...
        int[] seqUnitNum = seqUnitNumVar != null ? seqUnitNumVar.Value : new int[64];
        ...
        for (int i = 1; i <= numPerLayer; i++)
        {
            ...
            // Draw line on specified side of box based on LabelOrient, UnitR, and Rot180
            // LabelOrient: Bit flags - Bit 1(2)=Bottom, Bit 2(4)=Top, Bit 3(8)=Left, Bit 4(16)=Right (bit 0 not used)
            // UnitR: If != 0, rotate LabelOrient one position clockwise
            // Rot180 is 1-based (indices 1-64). Seq_UnitNum[k] contains the case number that Rot180[k] applies to
            // Example: if Rot180[1] = true and Seq_UnitNum[1] = 15, then case 15 should be rotated
            // To find which Rot180 index applies to case i, search for k where Seq_UnitNum[k] == i
            int rot180Index = 0; // 0 means not found
            // Search through Rot180 indices (1-64) to find which one maps to case i
            // Seq_UnitNum[k] contains the case number that Rot180[k] applies to
            // Example: if Seq_UnitNum[1] = 15, then Rot180[1] applies to case 15
            for (int k = 1; k < seqUnitNum.Length && k <= 64; k++)
            {
                if (seqUnitNum[k] == i)
                {
                    rot180Index = k;
            ...
            // Apply UnitR rotation (one position clockwise if UnitR != 0)
