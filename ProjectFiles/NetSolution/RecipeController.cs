...
public class RecipeController : BaseNetLogic
{
    ...
    [ExportMethod]
    public void SaveNewRecipeToDB(string RecipeName, CopyErrorPolicy ErrorPolicy)
    {
        if (SaveToDBTrigger != null)
            SaveToDBTrigger.Value = 0;
        ...
        try
        {
            ...
            if (store != null && editModel != null)
            {
                if (RecipeExistsInStore(store, schema, name))
                {
                    // Save Recipe
                    //schema.CopyToStoreRecipe(editModel.NodeId, name, ErrorPolicy);
                    SetFeedback(1, $"{GetLocalizedTextString("RecipeControllerNewRecipeAlreadyInDB")}");
                }
                else
                {
                    // Create recipe
    ...
    public void Export()
    {
        ...
        var csvPath = GetCSVExportFilePath();
    ...
    [ExportMethod]
    public void ExportPostSave(string RecipeName)
    {
        if (ExportTrigger != null)
            ExportTrigger.Value = 0;
        ...
        if (string.IsNullOrEmpty(csvPath))
        {
            SetFeedback(2, "Recipe Saved. Error with backup: please specify the output CSV file");
            return;
        }
        ...
        if (separator == '.')
        {
            SetFeedback(2, "Recipe Saved. Error with backup: CSV separator " + separator + " is not supported");
            return;
        }
        ...
        if (storeObject == null)
        {
            SetFeedback(2, "Recipe Saved. Error with backup: Store object not found");
            return;
        }
        ...
        {
            // No recipes or wrong result
            SetFeedback(2, $"Error with backup: No recipes found to export. Store {storeObject.BrowseName} has no recipes or an error occurred");
            return;
        }
        ...
        foreach (var columnName in header)
        {
            if (columnName.Contains(separator.ToString()))
            {
                SetFeedback(2, "Recipe Saved. Error with backup: the name of parameter " +
                    columnName + " contains the CSV separator " + separator);
                return;
            }
        }
        ...
        try
        {
            ...
            SetFeedback(1, "Recipe " + RecipeName + " Saved. Backup exported to " + csvPath);
        }
        catch (Exception e)
        {
            SetFeedback(2, "Recipe Saved. Error with backup: Unable to write CSV file: " + e.Message);
        }
    }


    [ExportMethod]
    public void CreateBackupFolder(DateTime dateTimeFromOptix)
    {
        Log.Info("Datetime: " + dateTimeFromOptix.ToString("yyyyMMMdd_HHmmss"));
        var originalCsvPath = GetCSVBackupFilePath();

        // Create a new CSV file path with the current date appended
        //string timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
        string newFolderPath = Path.Combine(
            Path.GetDirectoryName(originalCsvPath),
            dateTimeFromOptix.ToString("yyyyMMMdd_HHmmss")
        );
        ...
        if (!Directory.Exists(newFolderPath))
        {
            ...
            Log.Info("Folder created at: " + newFolderPath);
        }
    }

    [ExportMethod]
    public void ExportAndIterateFilePath2(DateTime dateTimeFromOptix)
    {
        string folderName = dateTimeFromOptix.ToString("yyyyMMMdd_HHmmss");
        Log.Info("In ExportAndIt2, folderName: " + folderName);
        ...
        var originalCsvPath = GetCSVBackupFilePath();
        string backupPath = Path.GetDirectoryName(originalCsvPath);
        string folderPath = Path.Combine(backupPath, folderName);

        if (string.IsNullOrEmpty(originalCsvPath))
        {
        ...
        {
            // No recipes or wrong result
            Log.Error($"No recipes found to export. Store {storeObject.BrowseName} has no recipes or an error occurred");
            return;
        }
        ...
        // Create a new CSV file path with the current date appended
        //string timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
        string newCsvPath = Path.Combine(
            folderPath,
            Path.GetFileNameWithoutExtension(originalCsvPath) + ".csv"
        );

        Log.Info("newCsvPath: " + newCsvPath);

        try
        {
            // Optionally copy the original CSV (if it exists and contains any pre-existing content) to the new file
            if (File.Exists(originalCsvPath))
            {
                File.Copy(originalCsvPath, newCsvPath);
            }

            // Write to the new file
            using (var csvWriter = new CSVFileWriter(newCsvPath) { FieldDelimiter = separator, WrapFields = wrapFields })
            {
                // Write header if not copying or if original file is empty
                if (!File.Exists(originalCsvPath) || new FileInfo(newCsvPath).Length == 0)
                {
                    csvWriter.WriteLine(header);
                }

                // For each recipe, write a line to the new CSV file
                for (var r = 0; r < rowCount; ++r)
                {
                    var currentRow = new string[columnCount];
            ...
            Log.Info("Recipes successfully exported to " + newCsvPath);
			SetFeedback(1, "Recipes successfully exported to " + newCsvPath);
        }
        catch (Exception e)
        {
    ...
    [ExportMethod]
    public void ExportAndIterateFilePath()
    {
        if (ExportTrigger != null)
            ExportTrigger.Value = 0;
        ...
        string newCsvPath = Path.Combine(
            Path.GetDirectoryName(originalCsvPath),
            Path.GetFileNameWithoutExtension(originalCsvPath) + "_" + timestamp + ".csv"
        );
    ...
    public void Import()
    {
        ...
        var csvPath = GetCSVImportFilePath();
    ...
    [ExportMethod]
    public void ImportStartupRecipes()
    {
        if (ImportTrigger != null)
            ImportTrigger.Value = 0;
        ...
        var csvPath = GetCSVStartupFilePath();
    ...
    [ExportMethod]
    public void ImportFromBackupFolder()
    {
        if (ImportTrigger != null)
            ImportTrigger.Value = 0;
        ...
        var csvPath = GetCSVBackupFolderPath();


        if (Environment.OSVersion.Platform == PlatformID.Win32NT)
        {
            if (LogicObject.Owner.BrowseName == "Basic")
            {
                csvPath += "\\BasicSnapshot.csv";
            }
            else if (LogicObject.Owner.BrowseName == "Sequence")
            {
                csvPath += "\\SeqSnapshot.csv";
            }
            else if (LogicObject.Owner.BrowseName == "SequenceF")
            {
                csvPath += "\\SeqFSnapshot.csv";
            }
        }
        else
        {
    ...
    private void SetFeedback(byte result, string message)
    {
        ...
        {
            task?.Dispose();
            task = new DelayedTask(() => { ResetFeedback(); }, 15000, LogicObject);
            task.Start();
        }
    }
    ...
    private bool RecipeExistsInStore(FTOptix.Store.Store store, RecipeSchema schema, string recipeName)
    {
        ...
        //Log.Info("rowcount > 0" + (rowCount > 0));

        return rowCount > 0;
    }

    [ExportMethod]
    public void DoesNewRecipeNameExist(string recipeName)
    {
        Log.Info("Checking new Recipe Nmae: " + recipeName);
        try
        {
            RecipeSchema schema = GetRecipeSchema();
            var store = GetRecipeStore(schema);
            Log.Info("Recipe Store: " + store.BrowseName + "Recipe Schema: " + schema.BrowseName);
            var newRecipeNameExists = Project.Current.GetVariable("Model/RecipeEdit/RecipeEditVariables/NewRecipeNameExists");
            if (recipeName == "Add New Name Here")
            {
                newRecipeNameExists.Value = true;

                Log.Info("Add new name, pretending it exists: " + recipeName);
            }
            else
            {
                newRecipeNameExists.Value = RecipeExistsInStore(store, schema, recipeName);

                Log.Info("newRecipeNameExists: " + newRecipeNameExists.Value);
            }

        }
        catch (Exception e)
        {
            Log.Error("RecipeNameParser", "Error encountered when grabbing names from db" + e.ToString());
        }
    ...
    private string GetCSVImportFilePath()
    {
        var csvPathVariable = LogicObject.GetVariable("ImportPath");
        if (csvPathVariable == null)
        {
            SetFeedback(2, "ImportPath variable not found");
            return "";
        }
    ...
    private string GetCSVExportFilePath()
    {
        var csvPathVariable = LogicObject.GetVariable("ExportPath");
        if (csvPathVariable == null)
        {
            SetFeedback(2, "ExportPath variable not found");
            return "";
        }
    ...
    private string GetCSVStartupFilePath()
    {
        var csvPathVariable = LogicObject.GetVariable("StartupPath");
        if (csvPathVariable == null)
        {
            SetFeedback(2, "StartupPath variable not found");
            return "";
        }
    ...
    private string GetCSVBackupFilePath()
    {
        var csvPathVariable = LogicObject.GetVariable("BackupPath");
        if (csvPathVariable == null)
        {
            SetFeedback(2, "BackupPath variable not found");
            return "";
        }

        return new ResourceUri(csvPathVariable.Value).Uri;
    }
    private string GetCSVBackupFolderPath()
    {
        var csvPathVariable = LogicObject.GetVariable("BackupImportFolder");
        if (csvPathVariable == null)
        {
            SetFeedback(2, "BackupImportFolder variable not found");
            return "";
        }
