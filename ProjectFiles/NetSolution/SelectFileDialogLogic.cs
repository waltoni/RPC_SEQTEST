...
public class SelectFileDialogLogic : BaseNetLogic
{
	public override void Start()
	{
        ...
        isBackup = widgetBackground.GetVariable("isBackup");
        if (isBackup == null)
		...
		if (!resourceUriHelper.IsFolderPathAllowed(startFolderPathResourceUri, accessFullFilesystemVariable.Value, accessNetworkDrivesVariable.Value))
		{
            ...
            LogicObject.GetVariable("SDNotRecognized").Value = false;
...
namespace FilesystemBrowserHelper
{
	public class ResourceUriHelper
	{
		...
		public string GetFTOptixStudioFormattedPath(ResourceUri oldResourceUri,
			string updatedPath, bool isBackup)
		{
			if (isBackup)
            {
				if (Environment.OSVersion.Platform == PlatformID.Win32NT)
				{
					updatedPath = updatedPath.Substring(0, updatedPath.LastIndexOf("\\")); // Remove the last character (a slash or a backslash)	
				}
                else
                {
                    updatedPath = updatedPath.Substring(0, updatedPath.LastIndexOf("/")); // Remove the last character (a slash or a backslash)	
